...
public class GenericExcelImportTest {
    ...
    @Test
    public void testReplaceExistingProcessesDoesNotCreateNewProcesses() throws Exception {
        File importFile = new File("src/test/resources/9923254553502466.xlsx");
        File almaRecordFile = new File("src/test/resources/9923254553502466.alma.xml");
        File importFolder = new File("src/test/resources/output");
        if (importFolder.exists()) {
        ...
        importFolder.mkdir();
        XMLConfiguration xmlConfig = new XMLConfiguration(new File("src/test/resources/plugin_intranda_import_excel_replace.xml"));
        Prefs prefs = new Prefs();
        prefs.loadPrefs("src/test/resources/edinburgh.xml");
        ...
        Process existingProcess = Mockito.mock(Process.class);
        StorageProviderInterface storage = Mockito.mock(StorageProviderInterface.class);
        try (MockedStatic<ProcessManager> processManager = Mockito.mockStatic(ProcessManager.class);
                MockedStatic<StorageProvider> storageProvider = Mockito.mockStatic(StorageProvider.class)) {
            processManager.when(() -> ProcessManager.getProcessByExactTitle(Mockito.anyString())).thenReturn(existingProcess);
            storageProvider.when(StorageProvider::getInstance).thenReturn(storage);

            List<ImportObject> importFiles = excelImport.generateFiles(records);

            // every record was written into an already existing process, so no ImportObject
            // must be returned - otherwise Goobi creates a duplicate process for it
            Assert.assertEquals(0, importFiles.size());
        }
    }
